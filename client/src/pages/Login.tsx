...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // This prevents users from being redirected back to the login/pricing page after logging in
      const forbiddenRedirects = ["/pricing", "/login", "/register", "/"];
      const targetPath = forbiddenRedirects.some(p => nextPath === p || nextPath.startsWith(p + "?"))
        ? "/dashboard"
        : nextPath;
  ...
  const handleSocialLogin = async (provider: "Google" | "GitHub" | "Apple") => {
    ...
    try {
      ...
      if (code === "auth/popup-closed-by-user") {
        toast.error("Login geannuleerd");
      } else if (code === "auth/account-exists-with-different-credential") {
        toast.error("Er bestaat al een account met dit e-mailadres");
      } else {
        toast.error(`Er is een fout opgetreden bij ${provider} login`);
      }
    } finally {
      setIsSubmitting(false);
    }
    ...
    <div className="min-h-screen bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950 flex items-center justify-center p-4 sm:p-6 lg:p-8">
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-zinc-400 text-sm sm:text-base">Log in om verder te gaan met ARCHON</p>
        </div>
        ...
        <div className="glass-card border border-white/10 rounded-2xl p-6 sm:p-8 shadow-2xl">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="relative py-2">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="px-3 bg-zinc-900/80 text-zinc-500">Of log in met</span>
              </div>
            </div>
          ...
          <div className="mt-6 text-center text-sm">
            <span className="text-zinc-400">Heb je nog geen account? </span>
