...
**Nieuwe feature:**
- `POST /api/offertes/generate` - AI Offerte generatie met foto analyse
- `isAiGenerated` status tracking
- Foto-gebaseerde afmeting detectie (m²)

**Features:**
- Google Gemini 1.5 Pro Vision integratie
- Spraakherkenning voor project details
- AI Geoptimaliseerd badge in UI
- Automatische koppeling van foto's aan offertes in Firestore
- Fallback naar basic generation bij missende API keys
...
- AI Capabilities: 20% → 95%

---

## Nog te doen (15% resterend)

### Prioriteit 1 (Belangrijk)
1. ~~**PDF Generatie** - Facturen en offertes als PDF genereren~~ ✅ DONE
2. ~~**Email Templates** - Templates voor facturen, welkomstmail, etc.~~ ✅ DONE
...
## 7. ✅ PDF Generatie & Email Templates (NEW)

### PDF API Endpoints
- `GET/POST /api/offertes/pdf` - Genereer offerte PDF
- `GET/POST /api/facturen/pdf` - Genereer factuur PDF

### Email Templates
- **Offerte Email** - Professionele offerte verzending
- **Factuur Email** - Factuur met betaalgegevens
- **Welkomst Email** - Onboarding nieuwe gebruikers
- **Herinnering Email** - Betalingsherinneringen

### Email API
- `POST /api/email/send-document` - Verstuur offerte/factuur/herinnering met templates

### Frontend Service
- `downloadOffertePdf()` - Download offerte als PDF
- `downloadFactuurPdf()` - Download factuur als PDF
- `sendOfferteEmail()` - Verstuur offerte via email
- `sendFactuurEmail()` - Verstuur factuur via email
- `sendReminderEmail()` - Verstuur betalingsherinnering
