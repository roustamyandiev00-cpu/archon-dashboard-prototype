...
2. ✅ **Firestore Schema** - Foto's worden nu opgeslagen (#4)
3. ✅ **handleAICreate** - Foto's worden doorgegeven (#3)

### P1 (BELANGRIJK - Binnen deze week)
4. ✅ **AIOfferteDialog imageUrls** - Consistente foto handling (#2)
5. ✅ **API imageUrls parameter** - Volledige functionaliteit (#1)
6. ✅ **Storage Rules** - Security (#8)

### P2 (NICE TO HAVE - Binnen deze maand)
7. ✅ **Speech Recognition** - Browser compatibiliteit en cleanup (#5)
8. ✅ **Error Handling** - Betere UX en badges (#6)
9. ✅ **Environment Variables** - Documentatie (#7)
...
### Direct te doen:
1. [x] Update `Offerte` interface met `images` en `dimensions` velden
2. [x] Update `handleAICreate` om `images` te accepteren en door te geven
3. [x] Update `AIOfferteDialog` om consistent `imageUrls` door te geven
4. [x] Update `generateOfferteWithAI` om `imageUrls` te gebruiken
5. [x] Update storage rules voor offerte foto's

### Later te doen:
6. [x] Voeg browser compatibiliteit toe voor speech recognition
7. [x] Verbeter error handling in API endpoints
8. [x] Update documentatie met nieuwe environment variables
