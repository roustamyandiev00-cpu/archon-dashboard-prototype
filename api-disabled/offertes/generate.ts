...
async function generateOfferteWithAI(
  ...
  // Images are now optional - if no images, generate from description only
...
Type werkzaamheden: ${projectType}`;
  ...
  } else if (!imageAnalysis.length) {
    // If no description and no images, ask AI to generate based on project type
    prompt += `\n\nGenereer een realistische beschrijving voor dit type project.`;
  }
...
Antwoord in JSON formaat:
{
  ...
  return {
    ...
    imageUrls: imageUrls,
    isAiGenerated: false,
    warning: !apiKey ? "AI Functies gedeactiveerd (geen API key gevonden)" : undefined
  };
}
